...
import (
	...
	"inference.networking.x-k8s.io/gateway-api-inference-extension/pkg/ext-proc/scheduling/algorithms"
)
...
var (
	...
	FilterTopK FilterGen = &filterGenImpl{
		name: FilterTopKName,
		getter: func(opt *FilterOption) filter {
			return filterTopKGetter(opt)
		},
		validator: func(fo *FilterOption) error {
			if fo == nil || fo.TopKBy == nil {
				return fmt.Errorf("topk filter requires topKBy and topK options")
			}
			switch *fo.TopKBy {
			case TopKByWaitingQueueSize, TopKByKVCacheUsagePercent:
			default:
				return fmt.Errorf("invalid topKByOption: %s", *fo.TopKBy)
			}
			return nil
		},
	}
